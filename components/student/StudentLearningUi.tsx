...
const resourceOrder = ["document", "slides", "download", "scripture_reference", "worksheet", "code_repository", "link", "other"];
...
export function SessionResources({ resources }: { resources: LearningResource[] }) {
  ...
  return <div className="space-y-6">{resourceOrder.flatMap((type) => {
    ...
    return [<section key={type}><h3 className="font-semibold text-[#071327]">{learningResourceTypeLabel(type)}</h3><ul className="mt-3 grid gap-3 md:grid-cols-2">{items.map((resource) => <li key={resource.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-700">{[fileTypeLabel(resource.fileName), learningResourceTypeLabel(resource.resourceType)].filter(Boolean).join(" · ")}</p><h4 className="mt-2 font-semibold text-[#071327]">{resource.title}</h4>{resource.description ? <p className="mt-2 text-sm leading-6 text-slate-600">{resource.description}</p> : null}{resource.externalUrl ? <a href={resource.externalUrl} target="_blank" rel="noreferrer noopener" className="mt-4 inline-flex min-h-11 items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-[#0b315c] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-700">Open Resource<span className="sr-only">: {resource.title} (opens in a new tab)</span></a> : resource.hasControlledFile ? <a href={`/api/student/session-resources/${resource.id}/download`} className="mt-4 inline-flex min-h-11 items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-[#0b315c] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-700">Open Material<span className="sr-only">: {resource.title}</span></a> : <p className="mt-4 text-sm font-medium text-slate-500">Reference provided for this class.</p>}</li>)}</ul></section>];
  })}</div>;
}
