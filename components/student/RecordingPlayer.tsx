...
export function CheckpointForm({ assignmentId, checkpoint }: { assignmentId: string; checkpoint: Record<string, unknown> }) {
  ...
  return <article id={`checkpoint-${String(checkpoint.id)}`} className="scroll-mt-6 rounded-2xl border border-slate-200 p-5"><h3 className="font-semibold text-[#071327]">Checkpoint {String(checkpoint.checkpoint_order)}</h3><p className="mt-1 text-sm text-slate-600">{String(checkpoint.title)}</p><div className="mt-4 space-y-4">{questions.map((question) => { const guidance = answerGuidance(question); return <form key={String(question.id)} onSubmit={(event) => void submit(event, String(question.id))} className="rounded-xl bg-slate-50 p-4"><label className="block text-sm font-medium text-[#071327]">{String(question.prompt)}{question.question_type !== "short_answer" && Array.isArray(question.options) ? <select name="answer" required className="mt-2 block min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3"><option value="">Choose an answer</option>{question.options.map((option) => <option key={String(option)} value={String(option)}>{String(option)}</option>)}</select> : <textarea name="answer" required minLength={numberOrUndefined(question.min_characters)} maxLength={numberOrUndefined(question.max_characters) ?? 5000} rows={question.response_format === "long_text" ? 6 : 3} className="mt-2 block w-full rounded-lg border border-slate-300 bg-white p-3" />}</label>{guidance ? <p className="mt-2 text-xs text-slate-600">{guidance}</p> : null}<button className="mt-3 rounded-lg bg-[#0b315c] px-4 py-2 text-sm font-semibold text-white">Submit response</button></form>; })}</div>{message ? <p role="status" className="mt-3 text-sm text-slate-700">{message}</p> : null}</article>;
}

function numberOrUndefined(value: unknown) { const number = Number(value); return Number.isInteger(number) && number > 0 ? number : undefined; }
function answerGuidance(question: Record<string, unknown>) {
  const parts: string[] = [];
  const minWords = numberOrUndefined(question.min_words); const maxWords = numberOrUndefined(question.max_words);
  const minCharacters = numberOrUndefined(question.min_characters); const maxCharacters = numberOrUndefined(question.max_characters);
  if (minWords && maxWords) parts.push(`${minWords}–${maxWords} words`); else if (minWords) parts.push(`At least ${minWords} words`); else if (maxWords) parts.push(`No more than ${maxWords} words`);
  if (minCharacters && maxCharacters) parts.push(`${minCharacters}–${maxCharacters} characters`); else if (minCharacters) parts.push(`At least ${minCharacters} characters`); else if (maxCharacters) parts.push(`No more than ${maxCharacters} characters`);
  return parts.join(" · ");
}
