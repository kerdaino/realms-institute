...
type Row = Record<string, unknown> & { id: string; assigned_delivery_route: DeliveryRoute; attendance_status: AttendanceStatus; finalized_at: string | null; roster_eligible?: boolean };
...
export function AttendanceRoster({ sessionId, initialRecord, scope }: { sessionId: string; initialRecord: RecordData; scope: "admin" | "facilitator" }) {
  ...
  return <div className="space-y-6 text-slate-900 [&_button.bg-white]:text-slate-900 [&_button.bg-white:disabled]:opacity-60 [&_input]:text-slate-900 [&_input::placeholder]:text-slate-400 [&_label]:text-slate-700 [&_select]:text-slate-900 [&_textarea]:text-slate-900 [&_textarea::placeholder]:text-slate-400">
    ...
    {!record.attendance.length ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-600">No student attendance records are available for this session yet.</div> : grouped.map(([route, rows]) => rows ? <section key={route} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
...
function AttendanceRow({ row, checks, scope, pending, onMutate, onAddCheck, onFinalize, onCorrect }: { row: Row; checks: Array<Record<string, unknown>>; scope: "admin" | "facilitator"; pending: boolean; onMutate: (row: Row, body: Record<string, unknown>) => Promise<void>; onAddCheck: (row: Row, body: Record<string, unknown>) => Promise<void>; onFinalize: (row: Row) => Promise<void>; onCorrect: (row: Row, status: AttendanceStatus, reason: string) => Promise<void> }) {
  ...
  const rosterEligible = row.roster_eligible !== false;
  ...
  return <article className="rounded-xl border border-slate-200 bg-slate-50 p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold text-[#071327]">{displayName(row)}</h3><p className="mt-1 text-xs font-medium text-slate-500">{String(student.student_number ?? "Student number unavailable")}</p>{!rosterEligible ? <span className="mt-2 inline-flex rounded-full border border-slate-300 bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">Removed from programme</span> : null}</div><div className="text-right"><span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-700">{attendanceStatusLabels[row.attendance_status]}</span><p className="mt-2 text-xs text-slate-500">Absence units: {Number(row.absence_weight ?? 0)}</p></div></div>
    {!row.finalized_at && rosterEligible && row.assigned_delivery_route === "PL" ? <div className="mt-4 grid gap-3 lg:grid-cols-2"><p className="rounded-lg bg-white p-3 text-xs leading-5 text-slate-600 lg:col-span-2">Timing guidance: within 15 minutes may remain Present with lateness recorded; more than 15 minutes is Late; missing more than 25% is Partial; missing more than 50% is Absent where actual timing evidence exists.</p>
