...
import { conditionalAdmissionDeadlineDays } from "@/lib/conditionalAdmission";
import { isFinancialRequirementSatisfied, registrationFinancialSummary } from "@/lib/scholarshipFinance";
import {
...
type AdmissionCommunicationEvent = { id: string; communication_type: string; delivery_status: string; provider_error: string | null; attempted_at: string; sent_at: string | null };
...
export function RegistrationDetail({ id }: { id: string }) {
  ...
  const [resendingAdmission, setResendingAdmission] = useState(false);
  ...
  const [latestAdmissionCommunication, setLatestAdmissionCommunication] = useState<AdmissionCommunicationEvent | null>(null);
  ...
  const loadRegistration = useCallback(async () => {
    ...
    setLatestAdmissionCommunication(body.latestAdmissionCommunication || null);
  ...
  async function resendConditionalAdmissionEmail() {
    setResendingAdmission(true);
    setResendMessage("");
    try {
      const response = await fetch(`/api/admin/registrations/${encodeURIComponent(id)}/conditional-admission-email`, { method: "POST" });
      const body = await response.json();
      if (!response.ok) throw new Error(body.message || "Conditional admission email could not be sent.");
      await loadRegistration();
      setResendMessage(body.message || "Conditional admission email sent.");
    } catch (error) {
      setResendMessage(error instanceof Error ? error.message : "Conditional admission email could not be sent.");
    } finally {
      setResendingAdmission(false);
    }
  }
  ...
  const currentFinancials = registrationFinancialSummary({
    ...
    fundingRoute: registration.funding_route,
  ...
  const historicalOfferDiffers = registration.admission_outstanding_amount !== null
    && currentFinancials.remainingDue !== null
    && Number(registration.admission_outstanding_amount) !== currentFinancials.remainingDue;
  ...
  return <div className="space-y-6">
    ...
    <Section title="E. Scholarship / Funding" id="scholarship-review">
      <Details items={[["Funding Route", registration.funding_route === "scholarship_request" ? "Scholarship Request" : "Self-Pay"], ["Scholarship Decision", scholarshipStatusLabels[registration.scholarship_status]], ["Normal Registration Fee", formatMoneyValue(registration.amount, registration.currency)], ["Approved Scholarship Support / Fee Waiver", currentFinancials.approvedSupport === null ? "Not approved" : formatMoneyValue(currentFinancials.approvedSupport, registration.currency)], ["Applicant Amount Due", currentFinancials.amountDue === null ? "Not yet determined" : formatMoneyValue(currentFinancials.amountDue, registration.currency)], ["Amount Paid", formatAmountPaid(registration)], ["Financial Requirement", currentFinancials.valid ? currentFinancials.financialRequirementStatus === "payment_required" ? "Payment required" : "Satisfied" : "Requires administrative review"], ["Reviewed At", formatDate(registration.scholarship_reviewed_at || null)], ["Reviewed By", registration.scholarship_reviewed_by || "Not reviewed"]]} />
    ...
    <Section title="G. Admission Review"><Details items={[["Admission Status", applicationStatusLabels[registration.application_status]], ["Current Financial Requirement", currentFinancials.valid ? currentFinancials.financialRequirementStatus === "payment_required" ? "Payment required" : "Satisfied" : "Requires administrative review"], ["Applicant Amount Due", currentFinancials.remainingDue === null ? "Could not establish safely" : formatMoneyValue(currentFinancials.remainingDue, registration.currency)], ["Conditional Admission Deadline", `${conditionalAdmissionDeadlineDays} calendar days after offer`], ["Offer Issued At", formatDate(registration.admission_offer_at)], ["Payment Deadline", formatDate(registration.admission_payment_deadline)], ["Outstanding at Offer", registration.admission_outstanding_amount === null ? "Not applicable" : formatMoneyValue(registration.admission_outstanding_amount, registration.currency)], ["Current Amount Due", currentFinancials.remainingDue === null ? "Could not establish safely" : formatMoneyValue(currentFinancials.remainingDue, registration.currency)], ["Payment Status", labelOrValue(paymentStatusLabels, registration.payment_status)], ["Admission Confirmed At", formatDate(registration.admission_confirmed_at)], ["Offer Lapsed At", formatDate(registration.admission_offer_lapsed_at)], ["Late Entry / Catch-Up", registration.late_entry_required ? `Required — flagged ${formatDate(registration.late_entry_flagged_at)}` : "Not flagged"], ["Reviewed At", formatDate(registration.reviewed_at)], ["Reviewed By", registration.reviewed_by || "Not reviewed"]]} />{historicalOfferDiffers ? <p className="mt-5 rounded-xl border border-red-300 bg-red-50 p-4 text-sm leading-6 text-red-950">The current amount due differs from the historical outstanding-at-offer snapshot. Review the authorised scholarship and verified payment history before sending another admission communication. The historical offer amount has not been changed.</p> : null}<p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">Admission remains separately controlled. Advanced-route or scholarship approval does not admit this applicant. Use conditional admission when academic approval is complete but verified payment remains outstanding.</p><form onSubmit={updateAdmissionStatus} className="mt-6 grid gap-4"><label className="grid gap-2 text-sm font-semibold text-slate-800"><span>Admission Status</span><select name="applicationStatus" defaultValue={registration.application_status} className="min-h-12 rounded-xl border border-slate-300 bg-white px-4 text-sm font-normal text-slate-950">{applicationStatuses.map((status) => <option key={status} value={status}>{applicationStatusLabels[status]}</option>)}</select></label><label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-[#f7f5ef] p-4 text-sm leading-6 text-slate-700"><input name="sendEmail" type="checkbox" className="mt-1 size-4 accent-[#a47720]" /><span>Send the appropriate branded admission/status email to the applicant</span></label><ActionButton submit disabled={Boolean(saving)}>{saving === "admission" ? "Saving..." : "Save Admission Status"}</ActionButton></form>{["conditional_admission_payment_outstanding", "admission_offer_lapsed_payment_outstanding"].includes(registration.application_status) ? <form onSubmit={extendPaymentDeadline} className="mt-7 grid gap-4 border-t border-slate-200 pt-6"><h3 className="font-semibold text-[#071327]">Extend Payment Deadline</h3><p className="text-sm leading-6 text-slate-600">A lapsed offer remains lapsed after extension. Reactivation requires a separate, deliberate admission-status decision.</p><label className="grid gap-2 text-sm font-semibold text-slate-800"><span>New Deadline</span><input required type="datetime-local" value={newPaymentDeadline} onChange={(event) => setNewPaymentDeadline(event.target.value)} className="min-h-12 rounded-xl border border-slate-300 px-4 font-normal" /></label><ReviewNote label="Reason for Extension" value={paymentDeadlineReason} onChange={setPaymentDeadlineReason} /><ActionButton submit disabled={extendingDeadline}>{extendingDeadline ? "Extending..." : "Extend Payment Deadline"}</ActionButton></form> : null}</Section>

    <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6" aria-labelledby="conditional-admission-email-heading"><h2 id="conditional-admission-email-heading" className="text-lg font-semibold text-[#071327]">Conditional Admission Email</h2><p className="mt-3 text-sm leading-6 text-slate-600">Send the current conditional offer again without changing the saved decision. The email uses a new expiring application-bound continuation link and the live server-derived amount and deadline.</p><div className="mt-5"><ActionButton tone="secondary" disabled={resendingAdmission || registration.application_status !== "conditional_admission_payment_outstanding"} onClick={() => void resendConditionalAdmissionEmail()}>{resendingAdmission ? "Sending..." : "Send / Resend Conditional Admission Email"}</ActionButton></div>{registration.application_status !== "conditional_admission_payment_outstanding" ? <p className="mt-3 text-sm text-slate-600">Available only while the application has an active conditional admission offer.</p> : null}{latestAdmissionCommunication ? <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700"><p><strong>Last attempt:</strong> {formatDate(latestAdmissionCommunication.attempted_at)}</p><p><strong>Result:</strong> {labelOrValue({}, latestAdmissionCommunication.delivery_status)}</p>{latestAdmissionCommunication.provider_error ? <p className="text-red-800"><strong>Failure:</strong> {latestAdmissionCommunication.provider_error}</p> : null}</div> : <p className="mt-4 text-sm text-slate-600">No conditional admission email attempt is recorded.</p>}</section>
