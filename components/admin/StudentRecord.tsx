...
export function StudentRecord({ initialRecord }: { initialRecord: StudentRecordData }) {
  ...
  async function sendPortalAccess() {
    if (!window.confirm(`Send a secure portal access email to ${student.email}?`)) return;
    setBusy(true); setMessage("");
    const response = await fetch(`/api/admin/students/${student.id}/portal-access`, { method: "POST" });
    const payload = await response.json(); setBusy(false);
    setMessage(response.ok ? "Portal access email sent." : payload.message || "Portal access could not be sent.");
  }
