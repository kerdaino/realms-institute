...
type RecordingRow = Record<string, unknown> & { purposes?: string[]; assignment_count?: number; in_progress_count?: number; integrity_review_count?: number; official_evidence_eligible?: boolean; recorded_route_eligible_count?: number; official_assignment_count?: number; approved_makeup_waiting_count?: number; makeup_assignment_count?: number };
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4">
      {recordings.map((recording) => { const session = sessionById.get(text(recording.class_session_id)); const purposes = recording.purposes ?? []; return <details key={text(recording.id)} open={Boolean(initialSessionId && initialSessionId === recording.class_session_id)} className="rounded-2xl border border-slate-200 bg-white p-5">
        <summary className="cursor-pointer list-none"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-amber-800">{session?.courseCode} · {session?.cohortCode}</p><h3 className="mt-1 font-semibold text-slate-900">{text(recording.title)}</h3><p className="mt-1 text-sm text-slate-600">{session?.title}</p></div><div className="flex flex-wrap gap-2"><StatusBadge value={text(recording.recording_status)} /><StatusBadge value={recording.quality_checked ? "quality_checked" : "quality_pending"} /></div></div><div className="mt-4 grid gap-2 text-xs text-slate-700 sm:grid-cols-3"><p className="rounded-lg bg-slate-50 p-3"><strong className="block text-slate-950">General Replay</strong>{recording.recording_status === "available" && recording.access_level === "enrolled_students" ? "Available" : "Not available"}</p><p className="rounded-lg bg-slate-50 p-3"><strong className="block text-slate-950">Official Recorded Attendance</strong>{recording.official_evidence_eligible ? "Eligible" : "Not eligible"} · {Number(recording.official_assignment_count ?? 0)} assigned / {Number(recording.recorded_route_eligible_count ?? 0)} eligible</p><p className="rounded-lg bg-slate-50 p-3"><strong className="block text-slate-950">Approved Make-Up</strong>{Number(recording.makeup_assignment_count ?? 0)} linked · {Number(recording.approved_makeup_waiting_count ?? 0)} waiting</p></div><p className="mt-3 text-xs text-slate-600">Current assignment purposes: {purposes.length ? purposes.map((code) => recordingPurposeLabels[code as RecordingPurposeCode] ?? code).join(" · ") : "None yet"} · {Number(recording.in_progress_count ?? 0)} open · {Number(recording.integrity_review_count ?? 0)} integrity review</p></summary>
        <form onSubmit={(event) => { event.preventDefault(); const prepared = bodyFrom(event.currentTarget); if (!prepared.ok) return setMessage(prepared.message); void send(`/api/admin/sessions/${text(recording.class_session_id)}/recordings/${text(recording.id)}`, "PATCH", prepared.body); }} className="mt-5 grid gap-4 border-t border-slate-200 pt-5 md:grid-cols-2">
          ...
          <div className="flex flex-wrap gap-3 md:col-span-2"><button disabled={busy} className="rounded-xl bg-[#0b315c] px-5 py-3 text-sm font-semibold text-white disabled:bg-slate-500">Save recording controls</button>{recording.official_evidence_eligible && Number(recording.recorded_route_eligible_count ?? 0) > 0 ? <button type="button" disabled={busy} onClick={() => void send(`/api/admin/sessions/${text(recording.class_session_id)}/recorded-learning`, "POST", {})} className="rounded-xl border border-emerald-600 bg-emerald-50 px-5 py-3 text-sm font-semibold text-emerald-900 disabled:opacity-60">Initialize / reconcile official assignments</button> : null}{session ? <Link href={`/admin/sessions/${session.id}#recorded-learning`} className="rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-800">Open requirements</Link> : null}</div>
        </form>
      </details>; })}
    </div>
  </div>;
