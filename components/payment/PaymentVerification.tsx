...
type State = { kind: "loading" | "success" | "failed"; reference: string; registrationMatched?: boolean; registration?: PaymentRegistrationDetails; payment?: PaymentDetails; customerEmail?: string; registrationSave?: RegistrationSave; emailStatus?: EmailStatus; message?: string };
...
export function PaymentVerification({ reference }: { reference: string }) {
  ...
  if (state.kind === "failed") return <Status title="Payment Not Confirmed" copy={`We could not confirm this payment. If you were debited, please contact REALMS Institute at ${contactEmail} with your payment reference.`} reference={state.reference} />;
  return <Status title="Payment Confirmed — Application Received" copy="Your registration payment has been confirmed and your application has been received. REALMS Institute will review your application, confirm your approved discipleship route, and contact you by email with your admission/onboarding status and next steps." reference={state.reference} registration={state.registrationMatched ? state.registration : undefined} payment={state.payment} customerEmail={state.registrationMatched ? undefined : state.customerEmail} registrationSave={state.registrationSave} emailStatus={state.emailStatus} showWhatsAppNextStep />;
}

function Status({ title, copy, reference, note, registration, payment, customerEmail, registrationSave, emailStatus, showWhatsAppNextStep = false }: { title: string; copy: string; reference?: string; note?: string; registration?: PaymentRegistrationDetails; payment?: PaymentDetails; customerEmail?: string; registrationSave?: RegistrationSave; emailStatus?: EmailStatus; showWhatsAppNextStep?: boolean }) {
