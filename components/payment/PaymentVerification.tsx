...
type State = { kind: "loading" | "success" | "failed"; reference: string; registration?: RegistrationDetails; payment?: PaymentDetails; message?: string };
...
export function PaymentVerification({ reference }: { reference: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/paystack/verify?reference=${encodeURIComponent(reference)}`, { signal: controller.signal })
      ...
      .then(({ ok, body }) => {
        if (ok && body.status === "success") setState({ kind: "success", reference, registration: body.metadata?.registration, payment: body.payment });
        else setState({ kind: "failed", reference, message: body.error || body.message });
      })
      .catch((error) => { if (error.name !== "AbortError") setState({ kind: "failed", reference, message: "Verification is temporarily unavailable." }); });
    return () => controller.abort();
  }, [reference]);

  if (!reference) return <Status title="Payment Not Confirmed" copy="We could not confirm this payment. If you were debited, please contact REALMS Institute with your payment reference." reference="Not provided" note="A payment reference was not provided." />;
  ...
  return <Status title="Registration Payment Successful" copy="Your registration payment has been confirmed. REALMS Institute will contact you with onboarding details for the next cohort." reference={state.reference} registration={state.registration} payment={state.payment} />;
}

function Status({ title, copy, reference, note, registration, payment }: { title: string; copy: string; reference?: string; note?: string; registration?: RegistrationDetails; payment?: PaymentDetails }) {
  return <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 sm:p-10"><h2 className="text-3xl font-semibold tracking-tight text-[#071327]">{title}</h2><p className="mt-4 max-w-2xl leading-7 text-slate-600">{copy}</p>{reference ? <p className="mt-6 rounded-xl bg-slate-100 p-4 text-sm text-slate-700"><strong>Payment reference:</strong> <span className="break-all">{reference}</span></p> : null}{note ? <p className="mt-3 text-sm text-red-700">{note}</p> : null}{registration ? <dl className="mt-8 grid gap-4 border-t border-slate-200 pt-7 sm:grid-cols-2"><Detail label="Applicant name" value={registration.fullName} /><Detail label="Email" value={registration.email} /><Detail label="WhatsApp" value={registration.whatsapp} /><Detail label="Learning mode" value={registration.learningMode} /><Detail label="Skill pathway" value={registration.skillPathway} />{payment ? <Detail label="Amount paid" value={payment.display} /> : null}</dl> : null}</div>;
}
