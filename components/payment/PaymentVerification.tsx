...
export function PaymentVerification({ reference }: { reference: string }) {
  ...
  return <Status title="Payment Confirmed — Application Received" copy="Your registration payment has been confirmed and your application has been received. REALMS Institute will review your application and contact you by email with your admission/onboarding status and next steps." reference={state.reference} registration={state.registrationMatched ? state.registration : undefined} payment={state.payment} customerEmail={state.registrationMatched ? undefined : state.customerEmail} registrationSave={state.registrationSave} emailStatus={state.emailStatus} showWhatsAppNextStep />;
}
