...
export async function fetchFacilitatorEngagementIndicators(supabase: SupabaseClient, offeringIds: string[]) {
  ...
  const [submissions, attempts] = await Promise.all([
    ...
    quizIds.length ? supabase.from("quiz_attempts").select("quiz_id, course_enrollment_id, attempt_status, passed").in("quiz_id", quizIds).in("course_enrollment_id", ids) : { data: [], error: null },
  ]);
  fail("Assignment indicators could not be loaded.", submissions.error); fail("Quiz indicators could not be loaded.", attempts.error);
  ...
  return enrollments.map((enrollment) => {
    ...
    const quizConcerns = quizRows.filter((quiz) => { const rows = (attempts.data ?? []).filter((item) => item.quiz_id === quiz.id && item.course_enrollment_id === id); return !rows.some((item) => item.passed === true) && !rows.some((item) => ["submitted", "awaiting_review"].includes(item.attempt_status)) && rows.filter((item) => item.attempt_status === "graded").length >= Number(quiz.max_attempts); }).length;
