...
export async function fetchAdminAbsenceMakeup(supabase: SupabaseClient, filters: Record<string, string | undefined> = {}) {
  ...
  const standaloneResult = await supabase.from("makeup_requirements").select("*, class_sessions(id, title, scheduled_start_at, cohort_course_id, cohort_courses(id, courses(id, code, title), cohorts(id, code, name))), course_enrollments(id, student_enrollments(id, enrolled_at, students(id, student_number, legal_name, preferred_name, email)))").is("absence_request_id", null).order("created_at", { ascending: false }).limit(5000); if (standaloneResult.error) throw new LmsAdminDataError("Standalone make-up records could not be loaded.");
  ...
  const filteredStandalone = (standaloneResult.data ?? []).filter((row) => { const student = relation(relation(row.course_enrollments).student_enrollments).students; if (filters.makeup && row.makeup_status !== filters.makeup) return false; if (filters.student && !JSON.stringify(student).toLowerCase().includes(filters.student.toLowerCase())) return false; return true; });
  const lateEntryMakeups = filteredStandalone.filter((row) => row.purpose_code === "LE-C");
  const standaloneMakeups = filteredStandalone.filter((row) => row.purpose_code !== "LE-C");
  const lateEntrantMap = new Map<string, { studentEnrollmentId: string; enrolledAt: string | null; student: Record<string, unknown>; required: number; completed: number; overdue: number }>();
  for (const row of lateEntryMakeups) { const enrollment = relation(row.course_enrollments); const studentEnrollment = relation(enrollment.student_enrollments); const key = String(studentEnrollment.id); const current = lateEntrantMap.get(key) ?? { studentEnrollmentId: key, enrolledAt: typeof studentEnrollment.enrolled_at === "string" ? studentEnrollment.enrolled_at : null, student: relation(studentEnrollment.students), required: 0, completed: 0, overdue: 0 }; current.required += 1; if (["completed", "late_complete"].includes(row.makeup_status)) current.completed += 1; else if (row.due_at && Date.parse(row.due_at) < Date.now()) current.overdue += 1; lateEntrantMap.set(key, current); }
  const lateEntrants = [...lateEntrantMap.values()];
  const metrics = { total: rows.length + filteredStandalone.length, pending: rows.filter((item) => ["submitted", "under_review"].includes(item.request_status)).length, informationRequired: rows.filter((item) => item.request_status === "more_information_required").length, approved: rows.filter((item) => item.request_status === "approved").length, declined: rows.filter((item) => item.request_status === "declined").length, makeupOutstanding: rows.filter((item) => { const makeup = relation(item.makeup_requirements); return makeup.id && !["completed", "late_complete", "waived", "cancelled"].includes(String(makeup.makeup_status)); }).length + filteredStandalone.filter((item) => !["completed", "late_complete", "waived", "cancelled"].includes(item.makeup_status)).length };
  return { rows, standaloneMakeups, lateEntryMakeups, lateEntrants, metrics };
}
