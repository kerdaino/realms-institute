...
export function sendApplicantConfirmationEmail(registration: SavedRegistration) {
  ...
  const text = `Dear ${registration.full_name},\n\nYour registration payment has been confirmed and your application has been received.\n\n${details.map(([label, value]) => `${label}: ${value ?? "Not provided"}`).join("\n")}\n\nREALMS Institute will review your registration and contact you with your admission/onboarding status and next steps.\n\nPlease keep your payment reference for your records.\n\nWith joy in Christ,\nREALMS Institute\n\nBringing the Will of the Father into the Earth Realm`;
  const html = `<div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;color:#0f172a;line-height:1.6"><p>Dear ${escapeHtml(registration.full_name)},</p><p>Your registration payment has been confirmed and your application has been received.</p><table style="border-collapse:collapse;width:100%;margin:20px 0">${rows(details)}</table><p>REALMS Institute will review your registration and contact you with your admission/onboarding status and next steps.</p><p>Please keep your payment reference for your records.</p><p>With joy in Christ,<br><strong>REALMS Institute</strong></p><p style="color:#475569;font-size:14px">Bringing the Will of the Father into the Earth Realm</p></div>`;
  return send(registration.email, "REALMS Institute Application Received", html, text);
}
...
export function sendAdminRegistrationNotification(registration: SavedRegistration) {
  ...
  const text = `A new registration payment has been confirmed and an application is ready for review.\n\n${details.map(([label, value]) => `${label}: ${value ?? "Not provided"}`).join("\n")}`;
  const html = `<div style="font-family:Arial,sans-serif;max-width:720px;margin:auto;color:#0f172a;line-height:1.5"><h2>New application received</h2><p>A new registration payment has been confirmed and an application is ready for review.</p><table style="border-collapse:collapse;width:100%">${rows(details)}</table></div>`;
