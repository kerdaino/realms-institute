...
begin
  ...
  values
    (p_key_hash, p_action, 1, current_time, current_time + make_interval(secs => p_window_seconds), current_time)
  on conflict (key_hash) do update
  set action = excluded.action,
      attempt_count = case when buckets.expires_at <= current_time then 1 else buckets.attempt_count + 1 end,
      window_started_at = case when buckets.expires_at <= current_time then current_time else buckets.window_started_at end,
      expires_at = case when buckets.expires_at <= current_time then current_time + make_interval(secs => p_window_seconds) else buckets.expires_at end,
  ...
  return jsonb_build_object(
    ...
    'retry_after_seconds', case
      ...
      else greatest(1, ceil(extract(epoch from bucket.expires_at - current_time)))::integer
    end
  );
end;
$$;
