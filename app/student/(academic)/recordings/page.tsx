...
import { recordingPurposeStudentCopy } from "@/lib/lms/recording";
...
function AssignmentList({ items }: { items: StudentRecordingAssignment[] }) {
  ...
  return <ul className="grid gap-4 lg:grid-cols-2">{items.map((item) => {
    const purposeCopy = recordingPurposeStudentCopy[item.purposeCode];
    const isRevision = item.purposeCode === "REV";
    const isZoomManual = item.recording.provider === "zoom" && item.recording.trackingMode === "manual_review";
    ...
    return <li key={item.id}><Link href={`/student/recordings/${item.id}`} className="block rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-amber-300"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-700">{item.course.code} · {purposeCopy.heading}</p><h2 className="mt-2 font-semibold text-[#071327]">{item.recording.title}</h2><p className="mt-1 text-sm text-slate-600">{item.session.title}</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">{isRevision ? "Optional revision" : humanizeStudentValue(item.displayStatus)}</span></div><p className="mt-3 text-sm leading-6 text-slate-600">{purposeCopy.description}</p>{!isZoomManual ? <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-amber-600" style={{ width: `${Math.min(100, item.progress.watchPercentage)}%` }} /></div> : null}{isRevision ? <><p className="mt-2 text-xs text-slate-600">{isZoomManual ? "Automatic revision viewing progress unavailable" : `Revision viewing progress: ${Math.floor(item.progress.watchPercentage)}% measured`} · Optional viewing</p><dl className="mt-4 grid grid-cols-2 gap-2 text-xs text-slate-600"><div><dt className="font-semibold text-slate-800">Minimum watch</dt><dd>No requirement</dd></div><div><dt className="font-semibold text-slate-800">Checkpoints</dt><dd>None required</dd></div><div><dt className="font-semibold text-slate-800">Deadline</dt><dd>No academic deadline</dd></div><div><dt className="font-semibold text-slate-800">Purpose</dt><dd>Revision only</dd></div></dl></> : <><p className="mt-2 text-xs text-slate-600">{isZoomManual ? "Automatic playback measurement unavailable · Staff verification required" : `${Math.floor(item.progress.watchPercentage)}% of required ${Number(item.effectiveRequirements.minWatchPercentage ?? 85)}% watch`}{item.dueAt ? ` · Due ${formatStudentDate(item.dueAt, true)}` : ""}</p><dl className="mt-4 grid grid-cols-2 gap-2 text-xs text-slate-600"><div><dt className="font-semibold text-slate-800">Checkpoints</dt><dd>{humanizeStudentValue(item.requirementSummary.checkpoints)}</dd></div><div><dt className="font-semibold text-slate-800">Quiz</dt><dd>{humanizeStudentValue(item.requirementSummary.quiz)}</dd></div><div><dt className="font-semibold text-slate-800">{supportRequirement[0]}</dt><dd>{humanizeStudentValue(supportRequirement[1])}</dd></div><div><dt className="font-semibold text-slate-800">Verification</dt><dd>{humanizeStudentValue(item.displayStatus)}</dd></div></dl></>}</Link></li>;
  })}</ul>;
}
