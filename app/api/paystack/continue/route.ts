...
import { initializePaymentContinuation } from "@/lib/paymentContinuation.server";
...
export async function POST(request: Request) {
  ...
  const token = body && typeof body === "object" && !Array.isArray(body) && typeof (body as Record<string, unknown>).token === "string"
    ...
    : "";
  if (!token || token.length > 1024) return NextResponse.json({ success: false, message: "A valid payment continuation link is required." }, { status: 400 });
  ...
  if (rateLimit.status === "unavailable") return NextResponse.json({ success: false, message: "Secure payment continuation is temporarily unavailable. Please wait a little and try again." }, { status: 503 });

  const result = await initializePaymentContinuation(token);
