...
import { withdrawStudentAbsenceRequest } from "@/lib/lms/absenceService";
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) { const { id } = await params; if (!isUuid(id)) return NextResponse.json({ message: "A valid absence request is required." }, { status: 400 }); try { const context = await requireStudentAbsenceApi(); const result = await withdrawStudentAbsenceRequest(context.supabase, context.user.id, id); await recordStudentMeaningfulActivity(context.supabase, context.user.id); return NextResponse.json(result); } catch (error) { return lmsApiError(error, "Absence request could not be withdrawn."); } }
