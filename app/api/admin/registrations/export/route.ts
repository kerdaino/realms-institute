...
function row(registration: AdminRegistration) {
  return [registration.full_name, registration.email, registration.whatsapp, registration.country, registration.city, registration.gender, registration.age_range, registration.church, registration.applicant_type, registration.requested_discipleship_route, registration.assigned_discipleship_route, registration.advanced_entry_status, registration.alumni_verification_status, registration.alumni_previous_cohort, registration.alumni_previous_email, registration.alumni_previous_phone, registration.alumni_student_id, registration.screening_status, registration.learning_mode, registration.skill_pathway, registration.reason, registration.referral_source, registration.fee_policy_consent ? "Yes" : "No", registration.computer_access_confirmed ? "Yes" : "No", registration.funding_route, registration.scholarship_status, registration.public_fee_display, registration.amount_display, registration.amount_paid, registration.amount, registration.currency, registration.payment_reference, registration.payment_status, registration.application_status, registration.admin_note, registration.reviewed_at, registration.reviewed_by, registration.paid_at, registration.created_at].map(csv).join(",");
}
