...
function row(registration: AdminRegistration) {
  return [registration.full_name, registration.email, registration.whatsapp, registration.country, registration.city, registration.gender, registration.age_range, registration.church, registration.learning_mode, registration.skill_pathway, registration.reason, registration.referral_source, registration.amount, registration.currency, registration.payment_reference, registration.payment_status, registration.paid_at, registration.created_at].map(csv).join(",");
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const headings = ["Full Name", "Email", "WhatsApp", "Country", "City", "Gender", "Age Range", "Church", "Learning Mode", "Skill Pathway", "Reason", "Referral Source", "Amount", "Currency", "Payment Reference", "Payment Status", "Paid At", "Created At"].map(csv).join(",");
