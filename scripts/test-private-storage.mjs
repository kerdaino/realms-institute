...
check("SQL creates all buckets private", () => { for (const bucket of Object.values(privateStorageBuckets)) assert.match(sql, new RegExp(`'${bucket}'[\\s\\S]{0,80}false`)); });
...
check("SQL blocks direct access with a restrictive storage policy", () => { assert.match(sql, /create policy realms_private_workflow_objects_server_only/); assert.match(sql, /as restrictive/); assert.match(sql, /bucket_id not in/); assert.doesNotMatch(sql, /using \(bucket_id in/); });
