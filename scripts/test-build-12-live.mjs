...
const buckets = await admin.storage.listBuckets(); assert.equal(buckets.error, null); assert.equal((buckets.data ?? []).some((bucket) => bucket.name === "institutional-awards" && bucket.public === false), true);
...
console.log("Build 12 live checks passed (14 tables, corrected alumni ownership columns, draft unsigned template, no public award read/write, and private award storage).");
